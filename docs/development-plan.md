...
- Done: first-use model download flow, verified catalog activation, active model recovery, verified-safe partial download cleanup, bounded shell/model/web-search paths, runtime request supervision, thread-scoped cancellation and approval failure feedback, symlink-safe workspace tools, progressive inspector surfaces, and native sandbox diagnostics.

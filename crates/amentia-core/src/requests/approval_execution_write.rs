...
use amentia_memory::MemoryEvent;
...
fn append_successful_write(
  ...
  workspace: &WorkspaceSummary,
  approval: &PendingApproval,
  write_result: amentia_tools::WriteFileResult,
) {
  ...
  events.push_workspace_change(amentia_storage::StoredWorkspaceChangeRecord {
  ...
  events.push_item(assistant_item(
    format!(
      "Amentia wrote {} in {} after your approval. Next: {}",
...
fn approved_write_continuation(relative_path: &str) -> ApprovedWriteContinuation {
  ...
  let message = match kind {
    "handoffSaved" => format!(
      "review {}, then ask Amentia to prepare a connector update or next-action list.",
      relative_path
    ),
    "summarySaved" => format!(
      "review {}, then ask Amentia to turn it into follow-up tasks if needed.",
      relative_path
    ),
    "noteSaved" => format!(
    ...
    _ => format!(
      "review {}, then ask Amentia to continue from the saved change.",
      relative_path
    ),
  };

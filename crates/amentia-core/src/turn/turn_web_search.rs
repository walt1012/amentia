...
use amentia_model_runtime::{GenerateRequest, ModelRole};
use amentia_protocol::TimelineItem;
use amentia_tools::{web_search_status, web_search_with_cancellation};
...
pub(super) fn model_confirms_web_search_candidate(
  ...
  let response = snapshot.model_runtime.generate(GenerateRequest {
    ...
    prompt: format!(
      "Choose whether Amentia should use web_search before answering.\n\

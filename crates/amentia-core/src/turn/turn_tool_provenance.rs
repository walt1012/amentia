...
pub(crate) const LOCAL_TOOL_SCHEMA: &str = "amentia.localTool.v1";
...
mod tests {
  ...
  fn local_tool_attributes_keep_legacy_tool_name_and_schema() {
    let attributes = local_tool_attributes(
      "web",
      "web_search",
      [("query".to_string(), "amentia release".to_string())],
    );
    ...
    assert_eq!(
      ...
      Some("amentia.localTool.v1")
    );
    assert_eq!(
      attributes.get("query").map(String::as_str),
      Some("amentia release")
    );
  }
  ...
  fn workspace_tool_attributes_infer_core_tool_kind() {
    let workspace = WorkspaceSummary {
      ...
      root_path: "/workspace/amentia".to_string(),
    };

...
pub(super) fn execute_write_turn(
  ...
  if intent.content.len() > write_file_max_bytes() {
    ...
    items.push(TimelineItem {
      ...
      content:
        "Amentia did not request approval because the proposed write is too large. Split it into smaller changes."
          .to_string(),
      attributes: None,
    });
    return;
  ...
  *pending_approval = Some(approval.clone());
  ...
  items.push(TimelineItem {
    ...
    title: "Assistant".to_string(),
    content: format!(
      "Amentia prepared a write for {} and is waiting for your approval.",
      intent.relative_path
    ),
    attributes: None,
  });
  ...
  match write_file(
    ...
    Ok(write_result) => {
      ...
      items.push(TimelineItem {
        ...
        title: "Assistant".to_string(),
        content: format!(
          "Amentia wrote {} in {} using approved workspace execution.",

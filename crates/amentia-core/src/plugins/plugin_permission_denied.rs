...
pub(crate) fn build_permission_denied_items(
  ...
  vec![
    TimelineItem {
      ...
      content: format!(
        "Amentia could not {blocked_action}{action_scope} because {permission_label} is not enabled."
      ),
...
fn permission_recovery_hint(permission: &str, blocked_action: &str, granted_by: &str) -> String {
  match permission {
    ...
    _ => format!(
      "Enable a plugin that grants `{permission}` before asking Amentia to {blocked_action}. Currently granted by: {granted_by}."
    ),
  }
}

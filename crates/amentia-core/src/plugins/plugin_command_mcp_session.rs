...
use super::plugin_command_runner_process::{
  insert_log_preview, insert_stdin_writer_attributes, plugin_runner_input_bytes, stderr_suffix,
  PluginRunnerProcessOutput, PLUGIN_RUNNER_GRACE_PERIOD, PLUGIN_RUNNER_OUTPUT_LIMIT,
...
pub(super) fn run_mcp_stdio_session(
  ...
  process
    ...
    .stdout(Stdio::piped())
    .stderr(Stdio::piped())
    .env("AMENTIA_PLUGIN_COMMAND_ID", &command.command_id)
    .env("AMENTIA_PLUGIN_ID", &command.plugin_id)
    .env("AMENTIA_PLUGIN_SANDBOX_DETAIL", request.sandbox.detail())
    .env("AMENTIA_PLUGIN_SOURCE_PATH", &command.source_path);

...
fn mcp_protocol_recovery_hint(attributes: &HashMap<String, String>) -> String {
  match attributes.get("mcpProtocolStatus").map(String::as_str) {
    ...
    Some("unsupportedContent") => {
      "Return MCP text content or structuredContent that Amentia can convert into text, timeline items, or memory notes."
        .to_string()
    }
    Some("emptyContent") => {
      "Return non-empty MCP text content or structuredContent that Amentia can convert into a useful plugin result."
        .to_string()
    }
    Some("emptyResult") => {
...
fn runner_setup_repair_hint(detail: &str) -> String {
  ...
  {
    return "Repair the workspace `.amentia` temporary directory so it is a real directory tree."
      .to_string();
  }

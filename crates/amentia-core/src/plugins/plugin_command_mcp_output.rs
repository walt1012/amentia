...
use super::plugin_command_mcp_output_protocol::{
  ...
  mcp_structured_content_looks_like_amentia_output, mcp_success_protocol_status,
  mcp_text_content_looks_like_amentia_output, missing_mcp_tool_response_message, scan_mcp_output,
...
pub(super) fn mcp_runner_output(
  ...
  if let Some(structured_content) = result.structured_content.as_ref() {
    if mcp_structured_content_looks_like_amentia_output(structured_content) {
      attributes.insert(
        "mcpStructuredContentStatus".to_string(),
        "amentiaOutputEnvelope".to_string(),
      );
      attributes.insert(
  ...
  if mcp_text_content_looks_like_amentia_output(&content) {
    attributes.insert(
  ...
  if mcp_command_requires_connector_workflow(command) {
    ...
    return Err(
      PluginRunnerFailure::with_output(
        -32054,
        format!(
          "MCP command `{}` is bound to a connector workflow and must return a Amentia output envelope.",
          command.command_id
        ),
        output.to_string(),
        String::new(),

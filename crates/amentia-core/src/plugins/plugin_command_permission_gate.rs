...
mod tests {
  ...
  fn command_permission_blockers_use_precise_plugin_actions() {
    let empty_input = amentia_plugin_host::PluginCommandEnvelopeEntry {
      envelope: "amentia.plugin.command.input".to_string(),
      fields: vec![],
    };
    let empty_output = amentia_plugin_host::PluginCommandEnvelopeEntry {
    ...
    let mcp_command = HostPluginCommandEntry {
      ...
      execution: Some(amentia_plugin_host::PluginCommandExecutionEntry {

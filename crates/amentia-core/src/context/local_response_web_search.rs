...
use amentia_tools::WebSearchResult;
...
pub(crate) fn summarize_declined_web_search_candidate(
  ...
  let prompt = format!(
    "You are Amentia, a concise local agent. Answer the user directly without using web_search.\n\
...
pub(crate) fn summarize_web_search_result(
  ...
  if results.is_empty() {
    let (summary, mut attributes) = generate_local_summary(
      model_runtime,
      format!(
        "You are Amentia, a concise local agent. Summarize a web search with no results.\nThread: {thread_title}\n{}\nQuery: {query}",
        format_memory_context_prompt(&memory_context)
      ),
      format!("Amentia searched the web and found no results for \"{}\".", query),
      &memory_context,
      None,
      cancellation,
    );
  ...
  let preview = results
    ...
    .join(", ");
  let observation_summary = format!(
    "Amentia searched the web and found {} results for \"{}\", including {}.",
    results.len(),
    query,
    preview
  );
  ...
  let prompt = format!(
    "You are Amentia, a concise local agent. Summarize these web search results in one or two sentences.\nThread: {thread_title}\n{}\nQuery: {query}\nResults:\n{}",
    format_memory_context_prompt(&memory_context),
    observation.text
  );
...
mod tests {
  ...
  fn web_search_summary_adds_source_note_even_when_urls_are_present() {
    ...
    assert!(!summary.contains("Sources: Amentia"));
  }

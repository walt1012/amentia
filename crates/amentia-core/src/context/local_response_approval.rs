...
pub(crate) fn summarize_denied_approval(
  ...
  let observation_summary = if action == "run_shell" {
    ...
    format!(
      "Amentia skipped the shell command `{}` because the approval was denied.",
      command
    )
  } else if action == "run_plugin_command" {
    let command = command.unwrap_or_default();
    format!(
      "Amentia skipped the plugin action `{}` because the approval was denied.",
      command
    )
  } else {
    format!(
      "Amentia skipped writing {} because the approval was denied.",
      relative_path
    )
  };
  let prompt = format!(
    "You are Amentia, a concise local cowork agent. Summarize a denied approval in one sentence.\nWorkspace: {workspace_name}\n{}\nAction: {}\nTarget: {}\nCommand: {}",

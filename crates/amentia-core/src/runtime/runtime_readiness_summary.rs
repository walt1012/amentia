...
pub(super) fn readiness_summary(input: ReadinessSummaryInput<'_>) -> String {
  ...
  match status {
    ...
    "setup_required" if !web_search_permission_ready => {
      "Enable Web Search so Amentia can retrieve current information when needed.".to_string()
    }
    "needs_approval" => {
      format!("Amentia is waiting on {pending_approval_count} approval(s) before continuing.")
    }
    "running" => {
...
mod tests {
  ...
  fn summary_uses_request_language_while_running() {
    ...
    assert_eq!(summary, "Amentia is working on 2 request(s).");

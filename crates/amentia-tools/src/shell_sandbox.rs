...
const SHELL_SANDBOX_TEMP_DIR: &str = ".amentia/sandbox-tmp";

pub(crate) struct ShellSandboxPlan {
  pub(crate) policy: amentia_sandbox::SandboxPolicy,
...
pub(crate) fn shell_sandbox_plan(workspace_root: &Path) -> ShellSandboxPlan {
  ...
  let status = amentia_sandbox::native_sandbox_status(&policy);
...
fn shell_sandbox_summary_from_status(
  status: amentia_sandbox::NativeSandboxStatus,
) -> ShellSandboxSummary {
  ShellSandboxSummary {
    mode: status.mode,
...
pub(crate) fn shell_sandbox_status(workspace_root: &Path) -> amentia_sandbox::NativeSandboxStatus {
  let policy = shell_sandbox_policy(workspace_root);
  amentia_sandbox::native_sandbox_status(&policy)
}

pub(crate) fn shell_sandbox_policy(workspace_root: &Path) -> amentia_sandbox::SandboxPolicy {
  amentia_sandbox::SandboxPolicy::workspace_read_write(workspace_root)

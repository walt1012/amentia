...
fn build_shell_command(
  ...
  _sandbox_policy: &amentia_sandbox::SandboxPolicy,
) -> Command {
...
fn build_shell_command(
  ...
  if amentia_sandbox::native_sandbox_available() {
    let profile = amentia_sandbox::macos_seatbelt_profile(sandbox_policy);
    let mut process = Command::new(amentia_sandbox::macos_sandbox_exec_path());
    process
      .arg("-p")
      .arg(profile)
      .arg("/bin/sh")
...
mod tests {
  ...
  fn shell_timeout_terminates_blocking_command() {
    ...
    let result = run_shell_with_timeout(
      "sleep 5",
      &workspace,
      &amentia_sandbox::SandboxPolicy::workspace_read_write(&workspace),
  ...
  fn sandbox_environment_routes_temporary_paths_to_workspace() {
    ...
    let temporary_root = workspace.join(".amentia").join("sandbox-tmp");
    let policy = amentia_sandbox::SandboxPolicy::workspace_read_write(&workspace)
      .with_temporary_root(&temporary_root);
    let mut command = Command::new("amentia-test");

...
const WEB_SEARCH_FIXTURE_ENABLED_ENV: &str = "AMENTIA_ENABLE_WEB_SEARCH_FIXTURE";
const WEB_SEARCH_FIXTURE_PATH_ENV: &str = "AMENTIA_WEB_SEARCH_FIXTURE_PATH";
...
mod tests {
  ...
  fn web_search_fixture_requires_explicit_test_gate() {
    ...
    let _path_guard = EnvVarGuard::set(
      ...
      std::ffi::OsStr::new("/tmp/amentia-fixture"),
    );
  ...
  fn web_search_fixture_runs_without_network() {
    ...
    let fixture_path = env::temp_dir().join(format!(
      "amentia-web-search-fixture-{}.html",
      std::process::id()
    ));
    std::fs::write(
      &fixture_path,
    ...
    let status = web_search_status();
    let results =
      web_search_with_cancellation("Amentia local model", 5, || false).expect("fixture web search");

    let _ = std::fs::remove_file(&fixture_path);
    assert_eq!(status.client, WEB_SEARCH_FIXTURE_CLIENT);
    assert!(status.available);
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].title, "Amentia fixture result");
    assert_eq!(results[0].url, "https://example.com/amentia");
  }

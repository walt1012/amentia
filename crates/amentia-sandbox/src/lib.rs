...
mod tests {
  ...
  fn profile_escapes_workspace_path() {
    let policy = SandboxPolicy::workspace_read_write("/tmp/Amentia \"Demo\"");
    let profile = macos_seatbelt_profile(&policy);

    assert!(profile.contains("(subpath \"/tmp/Amentia \\\"Demo\\\"\")"));
  }

  #[test]
  fn profile_escapes_control_characters_in_paths() {
    let policy = SandboxPolicy::workspace_read_write("/tmp/Amentia\nDemo\tRoot");
    let profile = macos_seatbelt_profile(&policy);

    assert!(!profile.contains("Amentia\nDemo"));
    assert!(profile.contains("(subpath \"/tmp/Amentia\\nDemo\\tRoot\")"));
  }

  #[test]
  fn profile_allows_temporary_root_explicitly() {
    let policy =
      SandboxPolicy::workspace_read_write("/Users/example/work").with_temporary_root("/tmp/amentia");
    let profile = macos_seatbelt_profile(&policy);
  ...
  fn prepare_workspace_temporary_root_clears_existing_directory() {
    ...
    let temporary_root = workspace.join(".amentia/tmp");
  ...
  fn prepare_workspace_temporary_root_rejects_parent_symlink_escape() {
    ...
    let temporary_root = workspace.join(".amentia/tmp/plugin");
    let parent = temporary_root.parent().expect("temporary parent");
    fs::create_dir_all(parent.parent().expect("amentia directory")).expect("amentia directory");
    fs::create_dir_all(&outside).expect("outside");
  ...
  fn status_reports_workspace_temporary_root() {
    let policy = SandboxPolicy::workspace_read_write("/Users/example/work")
      .with_temporary_root("/Users/example/work/.amentia/sandbox-tmp");
    let status = native_sandbox_status(&policy);

    assert_eq!(
      status.temporary_root.as_deref(),
      Some("/Users/example/work/.amentia/sandbox-tmp")
    );
    assert!(status
      .writable_roots
    ...
    assert!(status
      .writable_roots
      .contains(&"/Users/example/work/.amentia/sandbox-tmp".to_string()));
  }
  ...
  fn unique_temp_workspace(prefix: &str) -> PathBuf {
    let nonce = std::time::SystemTime::now()
      ...
      .expect("clock")
      .as_nanos();
    std::env::temp_dir().join(format!("amentia-sandbox-{prefix}-{nonce}"))
  }
}

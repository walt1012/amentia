...
pub(crate) fn model_metrics(
  ...
  if is_runtime_ready {
    metrics.insert(
      "invocationMode".to_string(),
      "bounded-llama-cpp".to_string(),
    );
...
fn install_hint(
  ...
  match readiness {
    ...
    "model_missing" => format!(
      "Place {} at {} or set AMENTIA_MODEL_PATH. Current binary candidate: {}.",
      file_name,
    ...
    "binary_missing" => format!(
      "Repair the packaged local inference backend or reinstall Amentia. Expected llama.cpp backend: {}. Current model candidate: {}.",
    ...
    "manifest_only" => format!(
      "Keep the manifest at {} and download {} in-app. If the backend is missing, repair or reinstall Amentia. Expected backend: {}.",
      manifest_path
        .map(display_path)
    ...
    _ => format!(
      "Use the in-app model manager to download {}. If the backend is missing, repair or reinstall Amentia. Expected manifest: {}. Expected backend: {}.",
      file_name,

...
fn discovery_roots_include_configured_model_directories() {
  ...
  let previous_model_pack_root = env::var("AMENTIA_MODEL_PACK_ROOT").ok();
  let previous_data_dir = env::var("AMENTIA_DATA_DIR").ok();

  env::set_var("AMENTIA_MODEL_PACK_ROOT", "C:/tmp/amentia-pack-root");
  env::set_var("AMENTIA_DATA_DIR", "C:/tmp/amentia-data");

  let roots = discovery_roots();

  match previous_model_pack_root {
    Some(value) => env::set_var("AMENTIA_MODEL_PACK_ROOT", value),
    None => env::remove_var("AMENTIA_MODEL_PACK_ROOT"),
  }
  match previous_data_dir {
    Some(value) => env::set_var("AMENTIA_DATA_DIR", value),
    None => env::remove_var("AMENTIA_DATA_DIR"),
  }

  assert!(roots.contains(&PathBuf::from("C:/tmp/amentia-pack-root")));
  assert!(roots.contains(&PathBuf::from("C:/tmp/amentia-data")));
  assert!(roots.contains(&PathBuf::from("C:/tmp/amentia-data").join("models")));
}
...
fn packaged_backend_requirement_disables_external_binary_fallbacks() {
  ...
  let previous_require = env::var("AMENTIA_REQUIRE_PACKAGED_LLAMACPP").ok();
  let previous_backend = env::var("AMENTIA_LLAMACPP_PATH").ok();

  env::set_var("AMENTIA_REQUIRE_PACKAGED_LLAMACPP", "1");
  env::remove_var("AMENTIA_LLAMACPP_PATH");
  assert_eq!(resolve_binary_path(), None);

  env::set_var("AMENTIA_LLAMACPP_PATH", &explicit_backend);
  assert_eq!(resolve_binary_path(), Some(explicit_backend.clone()));

  restore_env_var("AMENTIA_REQUIRE_PACKAGED_LLAMACPP", previous_require);
  restore_env_var("AMENTIA_LLAMACPP_PATH", previous_backend);
  remove_temp_directory(&temp_root);
}
...
fn bootstrap_pack_metadata_copies_manifest_and_readme_into_data_dir() {
  ...
  let previous_pack_root = env::var("AMENTIA_MODEL_PACK_ROOT").ok();
  let previous_data_dir = env::var("AMENTIA_DATA_DIR").ok();
  let previous_manifest = env::var("AMENTIA_MODEL_PACK_MANIFEST").ok();

  env::set_var("AMENTIA_MODEL_PACK_ROOT", &source_root);
  env::set_var("AMENTIA_DATA_DIR", &data_root);
  env::remove_var("AMENTIA_MODEL_PACK_MANIFEST");
  ...
  restore_env_var("AMENTIA_MODEL_PACK_ROOT", previous_pack_root);
  restore_env_var("AMENTIA_DATA_DIR", previous_data_dir);
  restore_env_var("AMENTIA_MODEL_PACK_MANIFEST", previous_manifest);
...
fn unique_temp_directory(prefix: &str) -> PathBuf {
  let nanos = SystemTime::now()
    ...
    .expect("system time")
    .as_nanos();
  env::temp_dir().join(format!("amentia-{prefix}-{nanos}"))
}

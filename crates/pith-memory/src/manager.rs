...
fn memory_note_parts(event: MemoryEvent) -> (String, String, String, String, Vec<String>) {
  match event {
    MemoryEvent::WorkspaceOpened {
      ...
      format!("Pith opened the workspace at {root_path}."),
      display_name,
      "workspace".to_string(),
    ...
    MemoryEvent::FileWritten {
      ...
      format!("Pith approved and wrote {relative_path} in {workspace_display_name}."),
      workspace_display_name,
      "approval".to_string(),
    ...
    MemoryEvent::ShellCommandRan {
      ...
      format!("Pith approved and ran `{command}` in {workspace_display_name}."),
      workspace_display_name,
      "approval".to_string(),
    ...
    MemoryEvent::ApprovalDenied { title, action } => (
      ...
      format!("Pith denied the pending action: {title}."),
      "global".to_string(),
      "approval".to_string(),
...
mod tests {
  ...
  fn manager_records_workspace_notes() {
    ...
    assert_eq!(notes[0].title, "Opened workspace pith");
  }

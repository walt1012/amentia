...
fn build_capability_registry_skips_disabled_and_invalid_plugins() {
  let plugins = vec![
    PluginCatalogEntry {
      ...
      manifest_path: "plugins/bundled/workspace-notes/amentia-plugin.json".to_string(),
    ...
    PluginCatalogEntry {
      ...
      manifest_path: "plugins/bundled/shell-recorder/amentia-plugin.json".to_string(),
      provenance: "bundled".to_string(),
      validation_error: None,
      validation_hint: None,
    },
...
fn build_command_registry_applies_default_execution_contract() {
  ...
  assert_eq!(execution.input.envelope, "amentia.plugin.command.input");
  ...
  assert_eq!(execution.output.envelope, "amentia.plugin.command.output");
...
fn build_command_registry_accepts_declared_connector_workflow() {
  ...
  let workflow = execution.workflow.as_ref().expect("workflow contract");
  assert_eq!(workflow.display_name, "Notion Create Page");
...
#[test]
fn build_command_registry_rejects_workflow_bound_to_wrong_connector() {
  let plugin_root = create_temp_plugin_root("command-workflow-wrong-connector");
  let plugin_dir = plugin_root.join("notion-runner");
  let commands_dir = plugin_dir.join("commands");
  fs::create_dir_all(&commands_dir).expect("create commands dir");
  fs::write(
  ...
  assert_eq!(
    commands[0].manifest_error.as_deref(),
    Some(
      "Plugin command `notion.publish-page` workflow `notion.create-page` is not bound to connector `notion`."
    )
  );
}
...
fn build_command_registry_skips_unsafe_capability_identifiers() {
  ...
  let plugin = PluginCatalogEntry {
    ...
    manifest_path: plugin_dir.join("amentia-plugin.json").display().to_string(),

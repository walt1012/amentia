...
pub fn install_plugin_bundle(
  ...
  Ok(crate::catalog::load_plugin_entry(
    destination_root.join("amentia-plugin.json"),
  ))
}
...
pub fn inspect_plugin_bundle(source_path: &Path) -> Result<PluginCatalogEntry> {
  ...
  let source_manifest_path = source_root.join("amentia-plugin.json");
...
fn resolve_plugin_source_root(path: &Path) -> Result<PathBuf> {
  ...
  if source_path.is_dir() {
    let manifest_path = source_path.join("amentia-plugin.json");
    if !manifest_path.is_file() {
      anyhow::bail!(
        "plugin directory {} does not contain amentia-plugin.json",
        source_path.display()
      );
    }
  ...
  if source_path
    ...
    .is_some_and(|name| name == "amentia-plugin.json")
  {
    return source_path
      .parent()
  ...
  anyhow::bail!("plugin source must be a plugin directory or amentia-plugin.json file");
}

fn ensure_single_root_manifest(source_root: &Path) -> Result<()> {
  let root_manifest = source_root.join("amentia-plugin.json");
...
fn reject_nested_plugin_manifests(directory: &Path, root_manifest: &Path) -> Result<()> {
    ...
    .with_context(|| format!("failed to read plugin directory {}", directory.display()))?
  {
    ...
    {
      anyhow::bail!(
        "plugin bundles cannot contain nested amentia-plugin.json manifests: {}",
        entry_path.display()
      );
    }
  }

...
use pith_tools::{run_shell_with_cancellation, shell_sandbox_summary};
...
fn shell_plan_summary(policy: &LocalChangeExecutionPolicy, workspace: &WorkspaceSummary) -> String {
  match policy {
    ...
    LocalChangeExecutionPolicy::Denied(_) => format!(
      "Check local execution mode and plugin permissions before running a shell command in {}.",
      workspace.display_name
    ),
  }
}
...
fn auto_approved_shell_handoff_attributes(
  ...
  policy: &LocalChangeExecutionPolicy,
  command: &str,
  result: &pith_tools::ShellCommandResult,

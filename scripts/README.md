...
- `package_macos_app.py`: builds and validates the x86_64 macOS app bundle and release zip; CI can also pass prebuilt app and runtime executables for faster parallel packaging, and records schema-versioned package metadata including daily-driver readiness provenance in `PithPackage.json`.
...
- `release_text.py`: generates and validates GitHub Release notes and the DMG root install guide from the release signing mode, including the daily-driver next-action path users should follow after install.

...
- `release_readiness.py`: prepares maintainer-facing Markdown and machine-readable JSON readiness reports before dispatching the release workflow, including tag preparation commands, exact source-commit CI lookup, the safe dry-run-first command, dry-run artifact verification, guarded post-acceptance publish command, release-candidate checklist, and expected dry-run evidence.

...
- `manual_acceptance_contract.py`: derives a structured fresh-Mac manual acceptance receipt template from downloaded release assets, then validates the filled receipt required before a visible ad-hoc prerelease.
...
- `release_evidence_contract.py`: validates internal dry-run and publish-rehearsal evidence artifacts, including structured readiness state, pre-dispatch gates, tag/CI lookup, guarded publish command content, release-plan action gates, first-install rehearsal gates, release-rehearsal JSON, rehearsal Markdown, manual acceptance receipt terms, dry-run installer asset integrity, cross-file release consistency, and asset-contract consistency, before workflow upload without adding extra public release assets.
...
- `release_state.py`: plans GitHub Release draft/prerelease safety for Developer ID, ad-hoc, and dry-run DMG builds, enforces tag/title identity, requires visible ad-hoc evidence to point at a validated manual acceptance receipt, revalidates release notes against the final publish state, and writes Markdown plus JSON release-plan evidence used in Actions.
...
- `test_release_evidence_contract.py`: checks internal release evidence artifact sets, structured readiness state, pre-dispatch gates, tag/CI lookup, guarded publish commands, release-plan action gates, first-install rehearsal gates, release-rehearsal JSON consistency, dry-run installer asset integrity, rehearsal and manual acceptance Markdown content, cross-file agreement, and missing or extra file rejection.
...
- `validate_workflows.py`: validates GitHub Actions structure so checkout credentials, internal artifact retention, CI lane splits, package dependencies, shared release inputs, evidence-before-visibility ordering, final Release tag/source-commit checks, user-facing installer uploads, and release assets do not regress.

...
- `smoke_launch_macos_app.py`: launches the packaged `Pith.app` on macOS CI with isolated app support, probes the packaged runtime protocol, and verifies app/runtime startup, workspace bootstrap, workspace search, web search readiness, sandbox readiness, thread creation, and runtime database initialization.

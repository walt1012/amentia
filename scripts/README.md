...
- `validate_workflows.py`: validates GitHub Actions structure so checkout credentials, internal artifact retention, CI lane splits, package dependencies, shared release inputs, evidence-step ordering, final Release tag/source-commit checks, user-facing installer uploads, and release assets do not regress.

...
- `release_evidence_contract.py`: validates internal dry-run and publish-rehearsal evidence artifacts, including structured readiness state, pre-dispatch checklist gates, tag/CI lookup, guarded publish command content, release-plan, release-rehearsal JSON, rehearsal Markdown, manual acceptance checklist terms, dry-run installer asset integrity, cross-file release consistency, and asset-contract consistency, before workflow upload without adding extra public release assets.

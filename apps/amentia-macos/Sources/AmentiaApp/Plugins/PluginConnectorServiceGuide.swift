  ...
  static func setupPrompt(connector: PluginConnectorSummary) -> String? {
    ...
    case "notion":
      return [
        ...
        "Amentia keeps the token local, passes it only to the local Notion plugin action, and does not claim OAuth yet.",
  ...
  static func missingTokenOrKeyWarning(connector: PluginConnectorSummary) -> String? {
    ...
    case "notion":
      return [
        ...
        "Amentia keeps the token local and passes it only to the Notion plugin action during approved runs.",
      ].joined(separator: " ")
    default:
      return nil
    }

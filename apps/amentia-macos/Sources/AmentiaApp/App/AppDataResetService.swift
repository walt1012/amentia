...
enum AppDataResetService {
  private static let connectorCredentialService = "app.amentia.plugin-connectors"
  ...
  private static func shouldRemoveSystemResidue(allowsNonDefaultRoot: Bool) -> Bool {
    let hasOverride = ProcessInfo.processInfo.environment["AMENTIA_APP_SUPPORT_DIR"]?.isEmpty == false

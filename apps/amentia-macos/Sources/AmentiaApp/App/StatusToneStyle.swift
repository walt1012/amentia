...
struct StatusPill: View {
  ...
  private var pillBackground: Color {
    if tone == .neutral {
      return AmentiaVisualStyle.panelBackground
    }
  ...
  private var pillBorder: Color {
    if tone == .neutral {
      return AmentiaVisualStyle.panelBorder
    }
...
struct SoftPanel: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      ...
      .shadow(color: AmentiaVisualStyle.panelShadow, radius: 8, x: 0, y: 3)
  }

  private var backgroundColor: Color {
    if isSelected {
      return AmentiaVisualStyle.selectedPanelBackground
    }
    if tone == .neutral {
      return AmentiaVisualStyle.panelBackground
    }

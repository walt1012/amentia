...
final class RuntimeBridge {
  ...
  private let connectionStateQueue = DispatchQueue(label: "amentia.runtime.bridge.connection-state")
  private var connectionStateValue: ConnectionState = .disconnected
  private let processStateQueue = DispatchQueue(label: "amentia.runtime.bridge.process-state")
  ...
  private func resolveRuntimeURL() throws -> URL {
    ...
    if let customPath = environment["AMENTIA_RUNTIME_PATH"], !customPath.isEmpty {
      return URL(fileURLWithPath: customPath)
    }

    if let bundledURL = Bundle.main.executableURL?
      .deletingLastPathComponent()
      .appendingPathComponent("amentia-runtime-bin"),
